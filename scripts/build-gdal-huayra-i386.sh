...
init_linux() {
	HDF4_LIBS="-lm"
	GDAL_CONFIGURE_FLAGS="--with-libkml=$PREFIX/ --with-=/usr/bin/curl-config --with-hdf5=yes --with-geotiff=/usr/lib --with-libtiff=/usr/lib --with-png=/usr/lib/i386-linux-gnu --with-sqlite3=$PREFIX --with-openjpeg=$PREFIX --with-ecw=$HOME/hexagon/ERDAS-ECW_JPEG_2000_SDK-5.2.1/Desktop_Read-Only/ --prefix=$PREFIX --with-local=/usr/local/"
