...
#	fi
	...
	GDAL_CONFIGURE_FLAGS="--with-expat=$PREFIX --with-libkml=$PREFIX  --with-geos=$PREFIX/bin/geos-config --with-curl=$PREFIX --with-hdf5=$PREFIX/bin --with-jpeg=$PREFIX --with-geotiff=$PREFIX --with-libtiff=$PREFIX --with-png=$PREFIX  --with-local=$PREFIX --with-libz=$PREFIX --with-sqlite3=$PREFIX --with-openjpeg=$PREFIX --prefix=$PREFIX"
}

init_linux() {
	HDF4_LIBS="-lm"
	GDAL_CONFIGURE_FLAGS="--with-libkml=$PREFIX/ --with-=/usr/bin/curl-config --with-hdf5=yes --with-geotiff=/usr/lib --with-libtiff=/usr/lib --with-png=/usr/lib/x86_64-linux-gnu --with-sqlite3=$PREFIX --with-openjpeg=$PREFIX --with-ecw=$HOME/hexagon/ERDAS-ECW_JPEG_2000_SDK-5.2.1/Desktop_Read-Only/ --prefix=$PREFIX --with-local=/usr/local/"
	...
	GDAL_COMPILER_FLAGS="-I${PREFIX}/include/kml/third_party/boost_1_34_1 -I/usr/include/geotiff"
...
if [ ${DO_GDAL:-"false"} = "true" ] ; then
   ...
   apply_patch "gdal-enhanced-support-webmercator.patch" "-d $OUTDIR" 1 || die
	if [ $WINDOWS ] ; then
